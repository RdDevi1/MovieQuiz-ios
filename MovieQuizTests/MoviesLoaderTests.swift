...
class MoviesLoaderTests: XCTestCase {
    ...
    func testSuccessLoading() throws {
        ...
        loader.loadMovies { result in
            ...
            case .success(let movies): // проверка что пришло два фильма из StubNetworkClient
                XCTAssertEqual(movies.items.count, 2)
                expectation.fulfill()
                
            case .failure(_):
                // не ожидаем, что пришла ошибка; если она появится, надо будет провалить тест
    ...
    struct StubNetworkClient: NetworkRouting {
        ...
        let emulateError: Bool // параметр, чтобы StubNetworkClient эмулировала либо ошибку сети, либо успешный ответ
